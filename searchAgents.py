...
class CapsuleSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        ## getCapsules() return a list of positions (x,y) of the remaining capsules.
        self.capsules = startingGameState.getCapsules()
        if len(self.capsules) == 0:
            self.hasCapsules = False
        else:
            self.hasCapsules = True
        self.foodList = self.foodPosition(self.food)


    def getGameStart(self):
        return self.start
        
    def hasNoCapsules(self):
        self.hasCapsules = False
    ...
    def getStartState(self):
        return (self.startState,self.foodList,self.hasCapsules)
    ...
    def isGoalState(self, state):
        ...
        remainFood = state[1]
        if (len(remainFood) == 0) and (state[2] == False):
            return True
        else:
            return False

    def eatAllFood(currentFood):
        num = 0
    ...
    ## Get a list of coordinates for all beans
    def foodPosition(self,currentFood):
        food = []
        for i, row in enumerate(currentFood):
    ...
    def getSuccessors(self, state):
        ...
        """
        "*** YOUR CODE HERE FOR TASK 3 ***"
        #The location of the capsule in this problem is constant initial information
        capsulePos = self.getCapsulePos()
        (x,y), currentFood, hasCapsules = state
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]: 
            ...
            if not self.walls[nextx][nexty]:
                current = util.manhattanDistance((x,y),capsulePos)
                next = util.manhattanDistance(pos,capsulePos)
                ## Is the next location you move to a capsule
                if (hasCapsules == True) :
                    if((x,y) == capsulePos):
                        hasCapsules = False
                    if (next > current):
                ...
                ## Is the next position to which it moves a bean?
                if pos in currentFood:
                    nextFood = [x for x in currentFood if x!= pos]
                
                nextState = ((nextx, nexty),nextFood, hasCapsules)
                ## stepcost =1
...
def capsuleProblemHeuristic(state, problem):
    ...
    """
    "*** YOUR CODE HERE FOR TASK 2 ***"
    position,foodList,hasCapsules = state
    distanceList = []
    distance = 0
    ...
    if len(foodList) == 0:
        return 0
    if (hasCapsules != True):
        #already ate capsules, go to eat dot
        while len(foodList) > 0:
            distanceList = findMinH(position,foodList)
            next = min(distanceList, key = lambda t: t[1])
            nextPos = next[0]
            position = nextPos
            distance += next[1] 
            foodList = [x for x in foodList if x!= nextPos]
    return distance

def findMinH(position,foodList):
    h = 0
    distanceList = []
    for i in foodList:
        closest,closestDis = closestFood(position,foodList)
        farthest,farthestDis = farthestFood(closest,foodList)
        h = closestDis + farthestDis
        distanceList.append((closest,h))
    return distanceList

'''
Find the closest food to current position of pacman
'''
def closestFood(position,foodList):
    closest = foodList[0]
    minDis = util.manhattanDistance(position,closest)        
    for i in foodList[1:]:
        temp = util.manhattanDistance(position,i)
        minDis = min(minDis,temp)
        closest = i
    return (closest,minDis)

'''
Find the farthest food to current food
'''
def farthestFood(food,foodList):
    foodList = [x for x in foodList if x!= food]
    if len(foodList) == 0:
        return (None,0)
    else:
        farthest = food
        maxDis = util.manhattanDistance(food,foodList[0])
        for i in foodList[1:]:
            temp = util.manhattanDistance(food,i)
            maxDis = max(maxDis,temp)
            farthest = i
        return (farthest,maxDis)
